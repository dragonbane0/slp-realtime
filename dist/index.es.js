import { merge, Subject as Subject$1, BehaviorSubject, ReplaySubject, fromEvent } from "rxjs";
import {
  Frames,
  isDead,
  Character,
...
import { get, sumBy, shuffle, last } from "lodash";
...
function filterOnlyFirstFrame() {
  return (source) => source.pipe(filter((frame) => frame.frame === Frames.FIRST));
}
...
function getStageInfo(stageId) {
  const s = stages.getStageInfo(stageId);
  if (!s) {
...
function extractPlayerNames(settings, metadata, playerIndex) {
  ...
  for (const index of indices) {
    ...
    const netplayName = get(metadata, ["players", index, "names", "netplay"], null);
    const netplayCode = get(metadata, ["players", index, "names", "code"], null);
    if (netplayName) {
...
})(DolphinPlaybackStatus || (DolphinPlaybackStatus = {}));
...
class DolphinOutput extends Writable {
  constructor(bufferOptions, opts) {
    super(opts);
    this.streamEndedSource = new Subject$1();
    this.playbackStatusSource = new Subject$1();
...
class DolphinLauncher {
  constructor(options) {
    ...
    this.dolphinRunningSource = new BehaviorSubject(false);
...
class ConversionEvents {
  constructor(stream) {
    ...
    this.state = new Map();
    this.conversionSource = new Subject$1();
...
(function (ComboEvent) {
  ...
  ComboEvent["End"] = "end";
})(ComboEvent$1 || (ComboEvent$1 = {}));
class ComboEvents {
  constructor(stream) {
    this.playerPermutations = new Array();
    this.state = new Map();
    this.combos = new Array();
    this.comboStartSource = new Subject$1();
    this.comboExtendSource = new Subject$1();
    this.comboEndSource = new Subject$1();
  ...
  processFrame(prevFrame, latestFrame) {
    this.playerPermutations.forEach((indices) => {
      ...
      switch (state.event) {
        case ComboEvent$1.Start:
          this.comboStartSource.next({
            combo: state.combo,
            settings: this.settings,
          });
          break;
        case ComboEvent$1.Extend:
          this.comboExtendSource.next({
            combo: state.combo,
            settings: this.settings,
          });
          break;
        case ComboEvent$1.End:
  ...
  if (opntIsDamaged || opntIsGrabbed) {
    ...
    if (opntDamageTaken) {
      ...
      if (!state.lastHitAnimation) {
        ...
        if (!comboStarted) {
          state.event = ComboEvent$1.Extend;
        }
      }
      if (state.move) {
        state.move.hitCount += 1;
    ...
    if (comboStarted) {
      state.event = ComboEvent$1.Start;
    }
  }
  if (!state.combo) {
 ...
 * @extends {Writable}
 */
class RxSlpStream extends SlpFileWriter {
  /**
  ...
  constructor(options, opts) {
    ...
    this.parser = new SlpParser({ strict: true }); // Strict mode will enable data validation
    this.messageSizeSource = new Subject$1();
    // Observables
    this.messageSize$ = this.messageSizeSource.asObservable();
    this.gameStart$ = fromEvent(this.parser, SlpParserEvent.SETTINGS).pipe(share());
    this.playerFrame$ = fromEvent(this.parser, SlpParserEvent.FINALIZED_FRAME).pipe(share());
    this.gameEnd$ = fromEvent(this.parser, SlpParserEvent.END).pipe(share());
    this.on(SlpStreamEvent.COMMAND, (data) => {
...
class SlpFolderStream extends RxSlpStream {
  constructor(options, opts) {
    ...
    this.startRequested$ = new Subject$1();
    this.stopRequested$ = new Subject$1();
    this.newFile$ = new BehaviorSubject(null);
    ...
    this.startRequested$
      .pipe(
        switchMap(([slpFolder, includeSubfolders]) => {
          ...
          return fromEvent(watcher, "add").pipe(
            share(),
...
export {
  ...
  findWinner,
  forAllPlayerIndices,
  generateDolphinQueue,
  generateDolphinQueuePayload,
  generateInputBitmask,
  getAllCharacters,
